...
export class OverrideToggle extends React.Component<{ value: boolean, onValue: (v: boolean) => void }> {
    render() {
        return <div className={"OverrideToggle" + (this.props.value ? " active" : "")} onClick={() => this.props.onValue(!this.props.value)}>
            {this.props.value ? <i className="fa fa-child" title="Manual input"/> : <i className="fa fa-cogs" title="Automatic"/>}
        </div>
    }
}
    ...
    render() {
        ...
        return <Card className="DimensionCard">
            <CardContent>
                ...
                {this.isExpanded && <Grid container spacing={16}>
                        <Grid item xs={8}><TextField label="Display name" value={dimension.displayName} onValue={this.onDisplayName} /></Grid>
                        <Grid item xs={4}><OverrideToggle value={dimension.props.displayName !== undefined} onValue={v => dimension.props.displayName = v ? dimension.displayName : undefined}/></Grid>
                        ...
                        <Grid item xs={4}><OverrideToggle value={dimension.props.unit !== undefined} onValue={v => dimension.props.unit = v ? dimension.unit : undefined}/></Grid>
                        <Grid item xs={8}><TextField label="Short (axis) unit" value={dimension.shortUnit} onValue={this.onShortUnit}/></Grid>
                        <Grid item xs={4}><OverrideToggle value={dimension.props.shortUnit !== undefined} onValue={v => dimension.props.shortUnit = v ? dimension.shortUnit : undefined}/></Grid>
                        ...
                        <Grid item xs={4}><OverrideToggle value={dimension.props.conversionFactor !== undefined} onValue={v => dimension.props.conversionFactor = v ? dimension.unitConversionFactor : undefined}/></Grid>
